...
export default async function AdminCategoriasPage() {
  ...
  return (
    <main className="space-y-3">
      <Link href="/protected/categorias/create" className="underline">Nueva categoría</Link>
      {categories.map((c) => (
        <div key={c.id} className="rounded border p-3 flex items-center justify-between gap-4">
          ...
          <div className="flex gap-2">
            <Link href={`/protected/categorias/${c.id}/edit`} className="underline">Editar</Link>
